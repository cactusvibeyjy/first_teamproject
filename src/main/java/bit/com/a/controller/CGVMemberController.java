...
public class CGVMemberController {
	...
	@RequestMapping(value = "moveNoMember.do", method = { RequestMethod.GET})
	public String moveNoMember() {
		logger.info("moveNoMember");
		return "noMember";
	}
	...
	public String modify(Model model, CGVMemberDto dto, HttpSession session) {
		logger.info("modify");
		CGVMemberDto login =(CGVMemberDto)session.getAttribute("login");
		dto.setId(login.getId());
		
		boolean isSuccess = cgvMemberService.modifyMember(dto);
	...
	public String delete(Model model, HttpSession session) {
		...
		boolean isSuccess = cgvMemberService.deleteMember(login.getId());
		if(isSuccess) session.invalidate();
