...
//0907 장, selected location 관련 추가
const selectedTheaterLocationInfo = document.querySelectorAll(
    '.selected-theater-location-info'
);
...
function selectListUiFunction(selectSeatListUlActive) {
    selectSeatListUlActive.forEach(li => {
        if (li.parentNode.classList.contains('select-seat-ul-normal')) {
            ...
            normalMoney = 14000 * normalNumber;
...
reserveSeat(); // 0908 장, 예약 좌석 목록 가져오기
...
function pageView(){
	for (let i = 0; i < 10; i++) {
        ...
        //3중포문을 사용하지 않기위해
        	mapping(input, i, j);
        	for(let k = 0; k<reserveSeats.length; k++){
        		if(input.value==reserveSeats[k]){
        			console.log(input.value +"="+ reserveSeats[k]);
        			input.disabled=true;
        			input.style.backgroundColor='#870517';
        		} 
        	}
        	div.append(input);
        //클릭시 이벤트
    	...
    	remainSeat.innerHTML = seat.length - reserveCnt;
    ...
    // 0908 장, 수정 및 추가
    	 selectedLocation.value =
        	selectedTheaterLocationInfo[0].innerHTML;
...
function inputClickEvent(input) {
    input.addEventListener('click', function(e) {
        ...
        remainSeat.innerHTML = seat.length - reserveCnt - selectedSeatsArray.length;
        // if (selectedSeatsArray.length > 4) {
        //     return;
        // }
    });
}
//0907 장, 예약좌석 목록 불러오기 추가
function reserveSeat() {
    $.ajax({
        url: 'reserveSeat.do',
        type: 'get',
        data: { "movieDate": theaterDate.innerHTML, "runningTime": theaterTime.innerHTML, 
        "selectedLocation": selectedTheaterLocationInfo[0].innerHTML,
        "selectedTheater": selectedTheaterPlaceInfo[0].innerHTML},
        success: function(data) {
        reserveSeats = data;
        reserveCnt = reserveSeats.length;
        pageView();
        },
    });
}
